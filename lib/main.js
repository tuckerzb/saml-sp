...
const KEY_CERT_HELP_TEXT =
  'Please generate a key-pair for the SP using the following openssl command:\n' +
...
function makeCertFileCoercer(type, description, helpText) {
  return function certFileCoercer(value) {
    ...
    throw new Error(
      'Invalid ' +
        description +
        ', not a valid cert/key or file path' +
        (helpText ? '\n' + helpText : '')
    );
  };
}
...
function processArgs(options) {
  ...
  return baseArgv
    ...
    .options({
      ...
      spCert: {
        description:
          'SP/RP Public Key Signature & Encryption Certificate (PEM)',
        string: true,
        required: false,
      ...
      spKey: {
        description:
          'SP/RP Private Key Signature & Decryption Certificate(PEM)',
        string: true,
        required: false,
    ...
    .example(
      '  $0 --idpMetaUrl https://example.okta.com/app/exknnoOGPcwWSnKUK0g3/sso/saml/metadata',
      'Dynamic IdP configuration with SAML Metadata'
    )
    .example(
      '  $0 --idpIssuer http://www.okta.com/exknnoOGPcwWSnKUK0g3 --idpSsoUrl https://example.okta.com/app/example_saml_2/exk7s3gpHWyQaKyFx0g4/sso/saml --idpCert ./idp-cert.pem',
      'Static IdP configuration with Public Key'
    )
    .example(
      '  $0 --idpIssuer http://www.okta.com/exknnoOGPcwWSnKUK0g3 --idpSsoUrl https://example.okta.com/app/example_saml_2/exk7s3gpHWyQaKyFx0g4/sso/saml --idpThumbprint ./idp-cert.pem',
...
function _runServer(argv) {
  IdPMetadata.fetch(argv.idpMetaUrl)
    ...
    .then(() => {
      ...
      httpServer.listen(process.env.PORT || argv.port, function () {
